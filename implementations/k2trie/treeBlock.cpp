...
// Global variables declared 'extern' in treeBlock.h.
...
// depth is the depth of the root of the block
{
   ...
   //uint16_t lowerB = 0.25 * nNodes;
   //uint16_t upperB = 0.75 * nNodes;
                       ...
                       uint64_t maxDepth, uint16_t curFlag)
{
   ...
   else if (nNodes + length - 1 <= maxNodes)
   {
      ...
      while (preorderOrigNode >= preorderNode)
      {
         ...
         dfuds[aux2 /*destNode.first*/] = dfuds[aux2 /*destNode.first*/] | (((dfuds[preorderOrigNode >> 2 /*origNode.first*/] >> 4 * (3 - (preorderOrigNode & 0x3) /*origNode.second*/)) & 0x000F) << aux);
         //prevNode(destNode);
         --preorderDestNode;
         //prevNode(origNode);
         --preorderOrigNode;
      ...
      for (uint16_t i = 1; i <= length; i++)
      {
         ...
         dfuds[aux2 /*origNode.first*/] = dfuds[aux2 /*origNode.first*/] | (symbol2NodeT[str[i]] << aux);
         ++nNodes;
         //nextNode(origNode);
         ++preorderOrigNode;
      }
   ...
   else
   {
      ...
      else
      {
         ...
         while (preorderSelectedNode /*absolutePosition(selectedNode)*/ < nNodes)
         {
            ...
            dfuds[originalSelectedNode.first] = dfuds[originalSelectedNode.first] & ~(0xF << auxOriginalS /*4*(3-originalSelectedNode.second)*/);

            dfuds[originalSelectedNode.first] = dfuds[originalSelectedNode.first] | (((dfuds[selectedNode.first] >> auxS /*4*(3-selectedNode.second)*/) & 0x000F) << auxOriginalS /*4*(3-originalSelectedNode.second)*/);

            dfuds[selectedNode.first] = dfuds[selectedNode.first] & ~(0xF << auxS /*4*(3-selectedNode.second)*/);
                                        ...
                                        uint16_t maxLevel, uint16_t &curFlag)
{
   ...
   while (curPreorder < nNodes && sTop >= 0 && diff < stack[0])
   {
      ...
      { //ptr != NULL && curFlag < nPtrs && curPreorder > ((blockPtr *)ptr)[curFlag].flag)
         ++curFlag;
         if (nPtrs == 0 || curFlag >= nPtrs)
            nextFlag = -1;
         else
                          ...
                          uint16_t &curFlag)
{
   ...
   if (soughtChild == (uint8_t)-1)
      return NULL_NODE; //treeNode((NODE_TYPE)-1, 0); // No such child to go down
...
uint64_t saveTrie(trieNode *t, char *filename)
{
   uint64_t size = sizeTrie(t);
