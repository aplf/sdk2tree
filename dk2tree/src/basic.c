...
#else
inline uint popcount (register int x) {
  return __popcount_tab[x & 0xff]  + __popcount_tab[(x >>  8) & 0xff]  + __popcount_tab[(x >> 16) & 0xff] + __popcount_tab[(x >> 24) & 0xff];
//  x = x - ((x >> 1) & 0x55555555);                    // reuse input as temporary
//  x = (x & 0x33333333) + ((x >> 2) & 0x33333333);     // temp
//  return ((x + (x >> 4) & 0xF0F0F0F) * 0x1010101) >> 24; // count
}
#endif
